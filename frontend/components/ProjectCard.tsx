import Image from 'next/image';
import { Project } from '../lib/sanity.queries';
import { urlFor } from '../lib/sanity';
import styles from './ProjectCard.module.css';

// ── SVG Icons ─────────────────────────────────────────────────

function IconExternal() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="14"
      height="14"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
      <polyline points="15 3 21 3 21 9" />
      <line x1="10" y1="14" x2="21" y2="3" />
    </svg>
  );
}

function IconGithub() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="14"
      height="14"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
    >
      <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
    </svg>
  );
}

// ── Props ──────────────────────────────────────────────────────

type ProjectCardProps =
  | { variant: 'featured'; project: Project }
  | { variant: 'regular'; project: Project }
  | { variant: 'more'; count: number };

// ── Sub-components ─────────────────────────────────────────────

function ProjectImage({ project, priority = false }: { project: Project; priority?: boolean }) {
  if (project.image) {
    const imageUrl = urlFor(project.image).width(800).height(450).url();
    return (
      <div className={styles.imageWrapper}>
        <Image
          src={imageUrl}
          alt={`Screenshot of ${project.title}`}
          fill
          sizes="(max-width: 768px) 100vw, 50vw"
          className={styles.image}
          priority={priority}
        />
      </div>
    );
  }
  return <div className={styles.imagePlaceholder} aria-hidden="true" />;
}

function TechChips({ technologies }: { technologies: string[] }) {
  return (
    <ul className={styles.chips} role="list" aria-label="Technologies used">
      {technologies.map((tech) => (
        <li key={tech} className={styles.chip}>
          {tech}
        </li>
      ))}
    </ul>
  );
}

function CardLinks({ liveUrl, github }: { liveUrl?: string; github?: string }) {
  if (!liveUrl && !github) return null;
  return (
    <div className={styles.links}>
      {liveUrl && (
        <a
          href={liveUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={styles.linkBtn}
          aria-label="Ver proyecto en vivo"
        >
          <IconExternal />
          <span>Ver proyecto</span>
        </a>
      )}
      {github && (
        <a
          href={github}
          target="_blank"
          rel="noopener noreferrer"
          className={`${styles.linkBtn} ${styles.linkBtnGhost}`}
          aria-label="Ver código en GitHub"
        >
          <IconGithub />
          <span>GitHub</span>
        </a>
      )}
    </div>
  );
}

// ── Main Component ─────────────────────────────────────────────

export default function ProjectCard(props: ProjectCardProps) {
  if (props.variant === 'more') {
    const { count } = props;
    return (
      <a
        href="/projects"
        className={`${styles.card} ${styles.cardMore}`}
        aria-label={`Ver ${count} proyectos más`}
      >
        <div className={styles.moreInner}>
          <span className={styles.moreCount}>+{count}</span>
          <span className={styles.moreLabel}>más proyectos</span>
          <span className={styles.moreArrow} aria-hidden="true">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            >
              <line x1="5" y1="12" x2="19" y2="12" />
              <polyline points="12 5 19 12 12 19" />
            </svg>
          </span>
        </div>
      </a>
    );
  }

  const { variant, project } = props;
  const isFeatured = variant === 'featured';

  return (
    <article className={`${styles.card} ${isFeatured ? styles.cardFeatured : styles.cardRegular}`}>
      {isFeatured && <ProjectImage project={project} priority />}

      <div className={styles.body}>
        <h3 className={styles.title}>{project.title}</h3>

        {project.description && (
          <p className={`${styles.description} ${!isFeatured ? styles.descriptionClamp : ''}`}>
            {project.description}
          </p>
        )}

        {project.stack && project.stack.length > 0 && (
          <TechChips technologies={project.stack} />
        )}

        <CardLinks liveUrl={project.liveUrl} github={project.github} />
      </div>

      {!isFeatured && project.image && (
        <div className={styles.regularImageWrapper}>
          <ProjectImage project={project} />
        </div>
      )}
    </article>
  );
}
